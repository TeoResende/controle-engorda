...
async def listar(sessao: SessaoDep) -> list[LoteComContagem]:
    ...
    contagem = (
        ...
        .where(Animal.fazenda_id == sessao.fazenda_id, Animal.status == StatusAnimal.ativo)
        .group_by(Animal.lote_id)
        .subquery()
    )
    stmt = (
        sessao.selecionar(Lote)
...
async def remover(lote_id: uuid.UUID, sessao: SessaoDep, ctx: EscritaDep) -> None:
    lote = await _obter(sessao, lote_id)
    # Animal fica sem lote (FK ON DELETE SET NULL), não some junto. Apagar um
    # lote não pode apagar histórico de peso de bicho nenhum.
    await sessao.session.delete(lote)
    await sessao.commit()
